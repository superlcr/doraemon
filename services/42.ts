...
import { DiscordRequest } from "../utils/request";
...
回答风格: 给出一句短而有力的核心回答(如签语或箴言), 纯文本格式, 不要多余的解释或背景故事. 
`;
...
export class AnswerCommand extends SlashCommand {
  ...
  override async handle(interaction: Interaction) {
    const userId = Reflect.get(interaction, "member").user.id;
    const channelId = Reflect.get(interaction, "channel_id");
    this.client.models
      .generateContent({
      ...
      .then(async (completion) => {
        await DiscordRequest(`channels/${channelId}/messages`, {
          method: "POST",
          body: { content: `<@${userId}> ${completion.text}` },
        });
      });
    return {
